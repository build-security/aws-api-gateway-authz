    ...
    "/pets": {
      "get": {
        ...
        "x-amazon-apigateway-integration": {
          ...
          "uri": "http://petstore.execute-api.eu-west-3.amazonaws.com/petstore/pets",
          "passthroughBehavior": "when_no_match",
          "httpMethod": "GET",
          "type": "http"
        }
    ...
    "/pets/{petId}": {
      "get": {
        ...
        "x-amazon-apigateway-integration": {
          ...
          "uri": "http://petstore.execute-api.eu-west-3.amazonaws.com/petstore/pets/{petId}",
          "passthroughBehavior": "when_no_match",
          "httpMethod": "GET",
          "type": "http"
        }
...
resource "aws_api_gateway_authorizer" "gateway-opa-auth" {
  name                   = "gateway-opa-auth"
  rest_api_id            = aws_api_gateway_rest_api.petstore_api.id
  authorizer_uri         = aws_lambda_function.lambda_opa_auth.invoke_arn
  type                   = "REQUEST"
  identity_source        = "method.request.header.user"
  authorizer_result_ttl_in_seconds = "0"
}

resource "aws_api_gateway_gateway_response" "opa_auth_deny" {
  rest_api_id   = aws_api_gateway_rest_api.petstore_api.id
  status_code   = "403"
  response_type = "ACCESS_DENIED"

  response_templates = {
    "application/json" = "{\"message\":$context.authorizer.denyMessage}"
  }
}

resource "aws_lambda_permission" "allow_api_gw_lambda_invoke" {
  statement_id  = "AllowExecutionFromAPIGW"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lambda_opa_auth.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.petstore_api.execution_arn}/authorizers/${aws_api_gateway_authorizer.gateway-opa-auth.id}"
}